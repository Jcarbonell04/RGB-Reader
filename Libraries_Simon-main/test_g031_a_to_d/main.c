// Basic STM32G031 A/D Demo
// Core clock is 64MHz, TIM2 timer interrupt enabled
//
// This requires a voltage source to measure on A4 (PA12)
// In the demo I used a 10K pot as a voltage div. with tap as vin
//
...
int main(void)
{
  ...
  { 
    ...
    // fire sample value out USART2
    {
      char buff[80];
      (void)sprintf (buff, "A/D Sample : %3.3X ", samp);
...
void _OneTimeInits(void)
{
  ...
  // starting up channel 16 (PA10 (A4)) on NUCLEO-G031K8
  _ADC_Init();
}
