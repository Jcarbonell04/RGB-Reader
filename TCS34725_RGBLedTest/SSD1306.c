...
// Sep 26 2024 - Migrated to STM32 version
...
///////////////////////////////////////////////////////
// SELECT THE APPROPRIATE DISPLAY SIZE IN THE HEADER!!!
///////////////////////////////////////////////////////
...
int _SSD1306_Command8 (unsigned char command)
{
  unsigned char buff [2];
  buff[0] = 0;
  buff[1] = command;

  if (_I2C1_StartWrite(_SSD1306_ADDRESS, 2, buff, _I2C_AutoEnd_Hardware))
      return -1;
...
int _SSD1306_Command16 (unsigned char commandA, unsigned char commandB)
{
  ...
  if (_I2C1_StartWrite(_SSD1306_ADDRESS, 3, buff, _I2C_AutoEnd_Hardware))
      return -1;
...
int _SSD1306_Data (unsigned char * data, unsigned int iCount)
{
  // if more than 255? bytes need to be sent, this works different on
  //  this platform, would need to operate I2C differently
  // currently only 128 byte banks are used, so we should be OK
  unsigned char buff [250];
  buff[0] = 0x40;   // write with more data
  memcpy (buff+1, data, iCount); 

  if (_I2C1_StartWrite(_SSD1306_ADDRESS, iCount + 1, buff, _I2C_AutoEnd_Hardware))
    return -1;
...
{
  _SSD1306_Command16 (0xA8, 0b10111111);   // set multiplex ratio P31 (default) (dim)
  //SSD1306_Command16 (0xA8, 0b10001111);   // set multiplex ratio P31 (16)
  
  _SSD1306_Command16 (0xD3, 0x00); // set display offset P31
  _SSD1306_Command8 (0x40);        // set display start line
  
  if (screen_dir)
  {
    _SSD1306_Command8 (0xA1);        // set segment remap
    _SSD1306_Command8 (0xC8);        // set com output map direction
  }
  else
  {
    _SSD1306_Command8 (0xA0);        // set segment remap
    _SSD1306_Command8 (0xC0);        // set com output map direction
  }
  
  _SSD1306_Command16 (0xDA, 0b00010010); // set com pins hardware config (alternative) (default?)

  _SSD1306_Command16 (0x81, 0x7F); // set contrast (1/2 level)
  _SSD1306_Command8 (0xA4);        // display on, use RAM
  _SSD1306_Command8 (0xA6);        // set normal display (1 == pixel on)
  
  _SSD1306_Command16 (0xD5, 0x80); // set display clock to defaults
  _SSD1306_Command16 (0x8D, 0x14); // this is critical, final pages (separate charge pump section)
  
  //_SSD1306_Command16 (0xD9, 0b00100010); // pre-charge period (default)
  //_SSD1306_Command16 (0xD9, 0b01000100); // pre-charge period (double?) not sure if this changed much
  
  _SSD1306_Command8 (0xAF);        // display on, normal mode
  
  _SSD1306_Command16 (0x20, 0x02); // page mode
  
  _SSD1306_Clear();                // ram will be scrambled eggs, so clear display
}
#endif
...
void _SSD1306_DispInit (_SSD1306_Orientation screen_dir)
{
  _SSD1306_Command16 (0xA8, 0x1F);   // set multiplex ratio P31 (default) (dim)
  ...
  _SSD1306_Command16 (0xDA, 0x02); // set com pins hardware config (alternative) (default?)
...
{
  _SSD1306_Command8 (0xAE);        // display sleep
}
...
void _SSD1306_Render (void)
{
  ...
  _SSD1306_Command8 (0x00);    // col 0
  _SSD1306_Command8 (0x10);    // col 0
  ...
  for (int i = 0; i < 8; ++i)
  {
    ...
    if (_DispDirty[i])
    {
      ...
      _DispDirty[i] = 0;
      _SSD1306_Command8 (0xB0 + i);  // set bank
      _SSD1306_Data(_DispBuff + i * 128, 128); // dump bank
    }
  }
}
#endif
...
void _SSD1306_SetPage (int page, unsigned char * buff)
{
    ...
    memcpy(_DispBuff + page * 128, buff, 128);
...
{
  if (IsInverse)
    return _SSD1306_Command8(0xA7);
  else
    return _SSD1306_Command8(0xA6);
}

int _SSD1306_Max (int iA, int iB)
{
  return (iA > iB) ? iA : iB;  
}

int _SSD1306_Min (int iA, int iB)
{
  return (iA < iB) ? iA : iB;
}

void _SSD1306_Order (int *iA, int *iB)
{
  if (*iA < *iB)
    return;
...
void _SSD1306_Circle (int iXS, int iYS, float fRad)
{
  ...
  {
    _SSD1306_SetPixel((int)(cos(fAng) * fRad + iXS), (int)(sin (fAng) * fRad + iYS));
  }
}
...
void _SSD1306_Line (int iXS, int iYS, int iXE, int iYE)
{
  ...
  int Steps = _SSD1306_Max(iXDisp, iYDisp);
  if (!Steps)
  {
    // this is a single pixel line
    _SSD1306_SetPixel(iXS, iYS);
    return;
  }
  ...
  {
    _SSD1306_SetPixel((int)fXPos, (int)fYPos);
...
void _SSD1306_CharXY (unsigned char iX, unsigned char iY, char disp)
{
  ...
  memcpy (_DispBuff + iStartIndex, _CharMap + (disp - 31) * 5, 5);
